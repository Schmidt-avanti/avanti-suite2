...
serve(async (req)=>{
  ...
  try {
    ...
    let eventsToProcess = Array.isArray(data) ? data : [
      data
    ];
    for (const event of eventsToProcess){
      ...
      try {
        const { data: emailData, error: emailError } = await supabase.from('inbound_emails').insert({
          ...
          to_emails: Array.isArray(event.to) ? event.to : [
            event.to || ''
          ],
      ...
      let isReply = false;
      // First, check if the subject contains any task ID in various formats like [TK123456], [HVN123], etc.
      const subject = event.subject || '';
      // This regex will match task IDs in formats like [TK123], [HVN456], [ABC-123], etc.
      const taskIdRegex = /\[([A-Za-z0-9]+-?[A-Za-z0-9]+)\]/;
      const taskIdMatch = subject.match(taskIdRegex);
      if (taskIdMatch && taskIdMatch[1]) {
        const extractedTaskId = taskIdMatch[1];
        console.log(`Extracted task identifier from subject: ${extractedTaskId}`);
        // Look up task using only the readable_id field
        const { data: taskData, error: taskError } = await supabase.from('tasks').select('id, readable_id').eq('readable_id', extractedTaskId).maybeSingle();
        if (!taskError && taskData) {
          relatedTaskId = taskData.id;
          isReply = true;
          console.log(`Found related task from readable_id: ${relatedTaskId} (readable_id: ${extractedTaskId})`);
        } else {
          console.log(`Task with readable_id ${extractedTaskId} not found in database`);
        }
      }
      // If we didn't find a task by subject ID, try the normal methods
      if (!relatedTaskId) {
        // First, try to find a related task using the In-Reply-To header
        if (event.in_reply_to) {
          isReply = true;
          const { data: threadData, error: threadError } = await supabase.from('email_threads').select('task_id').eq('message_id', event.in_reply_to).maybeSingle();
          if (threadError) {
        ...
        if (!relatedTaskId && event.reference_ids && event.reference_ids.length > 0) {
          ...
          for (const refId of event.reference_ids){
            const { data: refThreadData, error: refThreadError } = await supabase.from('email_threads').select('task_id').eq('message_id', refId).maybeSingle();
            if (refThreadError) {
        ...
        if (!relatedTaskId && event.subject_is_reply) {
          ...
          if (originalSubject) {
            const { data: subjectThreads, error: subjectError } = await supabase.from('email_threads').select('task_id').ilike('subject', originalSubject).order('created_at', {
              ascending: false
            }).limit(1);
            if (subjectError) {
      ...
      if (customerId) {
        ...
        const { data: customerData } = await supabase.from('customers').select('name').eq('id', customerId).single();
        ...
        if (relatedTaskId) {
          ...
          // Get the task status to see if we need to update it
          const { data: taskData } = await supabase.from('tasks').select('status').eq('id', relatedTaskId).single();
          const currentStatus = taskData?.status;
          // Update task status if it was waiting for customer response
          if (currentStatus === 'waiting_for_customer') {
            await supabase.from('tasks').update({
              status: 'in_progress'
            }).eq('id', relatedTaskId);
            console.log(`Updated task ${relatedTaskId} status from 'waiting_for_customer' to 'in_progress'`);
          }
          const { error: threadInsertError } = await supabase.from('email_threads').insert({
